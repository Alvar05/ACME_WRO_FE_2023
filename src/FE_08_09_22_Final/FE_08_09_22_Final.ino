...
HUSKYLENS camera;
HUSKYLENSResult bloque = camera.get(0);
...
void setup() {
  ...
  enviarI2C('S'); //Sends a flag to ProMicro to return the direction of the round ('A' for CCW and 'H' for CW)
  motorStartPower(10);
}
...
void loop() {
  ...
  if (vueltas == 0) { //Checks the direction in the first corner
    ...
    if (sensor.direccion == 'A') { //If direction is CCW, sets a variable to change the direction
      sentido = 180;
    }
  }
  else {
    while (distanciaTof < 700) { ////First turn (2nd and 3rd laps)
      if (tof.dataReady()) distanciaTof = tof.read(false);
      camera.requestBlocks();
      int dirOffset = 0;
      if (bloque.ID == 1 and bloque.height > 50) dirOffset = 30; // If the camera detects a red block set the direction offset to turn 30º right
      else if (bloque.ID == 2 and bloque.height > 50) dirOffset = -30; // If the camera detects a red block set the direction offset to turn 30º left
      M5.Lcd.setCursor(0, 0);
      M5.Lcd.print(distanciaTof + String("   "));
      sensorUpdate();
      direccion.write(pidGyro(0 + dirOffset, 1, 0, 0));
    }
  }
  motorResetCount("B");
  while (motorGetDistancia("B") < 80) {
    ...
    camera.requestBlocks();
    int dirOffset = 0;
    if (bloque.ID == 1 and bloque.height > 50) dirOffset = 30;
    else if (bloque.ID == 2 and bloque.height > 50) dirOffset = -30;
    M5.Lcd.setCursor(0, 0);
    M5.Lcd.print(distanciaTof + String("   "));
    speakerUpdate();
    direccion.write(pidGyro(90 + sentido + dirOffset, 1, 0, 0));
  }
  ...
  while (motorGetDistancia("B") < 80) {
    ...
    M5.Lcd.print(distanciaTof + String("   "));
    speakerUpdate();
    direccion.write(pidGyro(180 + dirOffset, 1, 0, 0));
  }
  ...
  while (motorGetDistancia("B") < 80) {
    ...
    M5.Lcd.print(distanciaTof + String("   "));
    speakerUpdate();
    direccion.write(pidGyro(270 - sentido + dirOffset, 1, 0, 0));
  }
