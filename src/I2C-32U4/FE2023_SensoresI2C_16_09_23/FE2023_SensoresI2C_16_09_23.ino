...
void loop() {
  ...
  if (!flagSPI) {
    ...
    if (command.leerSector) {
      ...
      filtrarBloquesHL(command.filtroCam);
      //i2c.bloqueCerca = readMedianaHL(indexBloques[0], 15, 'I');
      i2c.sector = readSectorMedianaLejos(15);
      lecturasCam++;
      huskylens.customText(i2c.sector + String("  "), 0, 0);
      //huskylens.customText(i2c.bloqueCerca + String("  "), 0, 30);
      //huskylens.customText(lecturasCam + String("           "), 0, 30);
    }
  }
  if (micros() > tiempoLED) {
