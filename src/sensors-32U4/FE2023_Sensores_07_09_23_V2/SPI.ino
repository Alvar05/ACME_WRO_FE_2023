void setupSPI() {
  ...
  SPI.attachInterrupt();
  SPDR = *(byte*) &sensor; // Carga el primer byte de la trama
}
...
void updateSPI() {
  if (haveData) {
    ...
    if (bitRead(command.motorFlags, 3) != distanceBitState) {
      ...
      //bool negative = bitRead(command.motor, 7); FUNCTION ONLY FOR POSITIVE SPEEDS
...
void imprimirComandos() { //Prints by Serial the sensor data
  Serial.print(command.startByte, DEC);
  Serial.print("  ");
  Serial.println(command.endByte, DEC);
  Serial.print("Motor= ");
  Serial.print(command.motor, BIN);
  Serial.print("  MotorFlags= ");
  Serial.print(command.motorFlags, BIN);
  Serial.print("  Flags= ");
  Serial.print(command.flags, BIN);
  Serial.print("  Dist= ");
  Serial.print(command.distance);
  Serial.print("  vMAX= ");
  Serial.println(command.vMaxDistance);
  for (uint8_t i = 0; i < sizeof(command.masComandos); i++) {
    Serial.print(command.masComandos[i] + String(" "));
  }
  Serial.println("\n\n");
}
