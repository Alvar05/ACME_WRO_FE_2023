...
long motorGetSlaveDistance(String motor) {                //Returns the distance in mm done by the tyre
...
int motorGetSpeed() {  // Returns the real velocity -100 - 100
  int vel = 0;
...
void motorStart(int velocidad) { // PID control for a certain velocity
  if (velocidad == 0) {
    motorStartPower(0);
    return;
  }
  float kp = 3, ki = 0.01, kd = 3;
...
void motorStart(int velocidad, bool freno) {
  if (velocidad == 0) {
    motorStartPower(0);
    return;
  }
  ...
  giro = proporcional + integral + derivativo;
  if (velocidad > 0) {
    if (freno) motorStartPower(constrain(velocidad - (int)giro, -20, 100));
    else motorStartPower(constrain(velocidad - (int)giro, 0, 100));
  }
  else if (velocidad < 0) {
    if (freno) motorStartPower(constrain(velocidad - (int)giro, -100, 20));
    else motorStartPower(constrain(velocidad - (int)giro, -100, 0));
  }
}

void motorStart(int velocidad, bool freno, float kp, float ki, float kd) {
  if (velocidad == 0) {
    motorStartPower(0);
    return;
  }
...
bool acelExpo(int velInicial, int velFinal) {          // Makes an acceleration and returns 1 when finished
  velInicial = constrain(velInicial, 2, 100);
  velFinal = constrain(velFinal, 0, 100);
  const float distInicial = (velInicial * MIN_ACCEL_DISTANCE) / 100;
...
bool desacelExpo(int velInicial, int velFinal) { // Makes a decceleration and returns 1 when finished
  ...
  const float distInicial = ((100 - velInicial) / 100.00) * MIN_DESACCEL_DISTANCE;
...
bool acelExpoCapado(int velInicial, int velMax, int velFinal) {          //Also returns 0 y 1 as the acelExpo() does
  velInicial = constrain(velInicial, 2, 100);
  const unsigned long distInicial = (velInicial * MIN_ACCEL_DISTANCE) / 100;
  long velocidad = ((distInicial + motorGetSlaveDistance("D") * 100) / MIN_ACCEL_DISTANCE);
  if (velocidad <= velFinal) {
    velocidad = constrain(velocidad, 5, velMax);
    if (velocidad >= V_MAX_PID) motorStartPower(velocidad);
    else motorStart(velocidad);
    return 0;
  }
...
bool desacelExpoCapado(int velInicial, int velMax, int velFinal) { //Same as acelExpoCapado but to decelerate
  velFinal = constrain(velFinal, 0, 100);
  if (velFinal == 0) {
    desacelBreak = true;
    velFinal = MIN_V_TO_BREAK;
  ...
  if (velocidad >= velFinal) {
    ...
    if (velocidad > V_MAX_PID) motorStartPower(velocidad);
    else motorStart(velocidad, true);
    return 0;
  }
  else {
