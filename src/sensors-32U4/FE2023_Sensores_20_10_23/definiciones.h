...
#define MIN_DESACCEL_DISTANCE 5000 // Minimum distance to go 100-0 safely
#define V_MAX_PID 80
...
const float diametroRueda = 29; // Tyre diameter in mm
...
#define MAX_DISTANCE_CENTRO 200 // Maximum distance (in cm) to ping the center US.
#define MAX_DISTANCE 90 // Maximum distance (in cm) to ping the lateral USs.
...
struct infoCommands { // 10 data bytes
  ...
  byte masComandos[3] = {2, 3, 4}; // Extra trash data to complete the number of bytes
  const byte endByte = ENDBYTE;
};
