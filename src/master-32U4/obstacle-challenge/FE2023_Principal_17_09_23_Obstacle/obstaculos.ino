  ...
  if (sentido == 1) { //////////////////////////////////////////////////////////////////////////////////////////////////////////// CW
    ...
    switch (sector) {
      case 1:
        ...
        setDireccionCamara(110);
        giroEnlace(450, direccion);
        activarHL();
        beep(100);
        intermitente('A');
        moveDistancia(200 + distAnterior - 200, 5, 5); // 200 es el inicio 0 teórico, por eso se resta
        while (!commandFinished) updateRobot();
        if (numSector == 0 and vuelta == 2) robotStop();
        beep(100);
        if (leer or vuelta == 0) {
          motorStartPower(0);
          long tiempoBloque = millis() + TIEMPO_BLOQUE;
          while (millis() < tiempoBloque) updateRobot();
          if (verdeCerca()) {
            casoBreak = 1;
      ...
      case 2:
        ...
        beep(100);
        intermitente('A');
        moveDistancia(200 + distAnterior - 200, 5, 5);
        while (!commandFinished) updateRobot();
        if (numSector == 0 and vuelta == 2) robotStop();
        if (leer or vuelta == 0) {
          motorStartPower(0);
      ...
      case 3:
        ...
        giroEnlace(450, direccion);
        intermitente('A');
        moveDistancia(0 + distAnterior - 199, vMaxSector, 5);
        while (!commandFinished) updateRobot();
      ...
      case 4:
        ...
        if (numSector == 0 and vuelta == 2) {
          moveDistancia(200 + distAnterior - 200, vMaxSector, 5);
          while (!commandFinished) updateRobot();
          robotStop();
        }
    ...
    switch (sector) { // CCW
      ...
      case 4:
        ...
        if (numSector == 0 and vuelta == 2) robotStop();
        moveDistancia(450, vMaxSector, vMinSector);
        intermitente('I');
        distAnterior = 800;
        while (!commandFinished) updateRobot();
        break;
